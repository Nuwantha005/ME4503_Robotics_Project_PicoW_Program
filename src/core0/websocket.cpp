...
static void base64_encode(const uint8_t* in, size_t in_len, char* out, size_t out_buf_len) {
    ...
    while (i < in_len) {
        ...
        if (j + 4 >= out_buf_len) break;   // safety
    ...
    for (size_t p = 0; p < pad && j > 0; p++) out[j - 1 - p + pad] = '=';
    out[j] = '\0';
}
...
void ws_feed_data(struct tcp_pcb* pcb, const uint8_t* data, size_t len) {
    if (pcb != s_ws_pcb || !s_ws_connected) return;
    ...
    while (s_rx_len - consumed >= 2) {
        ...
        if (len_byte < 126) {
            ...
            if (avail < 4) break;   // wait for more data
        ...
        if (avail < total_frame) break;  // incomplete frame, wait
...
bool ws_send_text(const char* text, size_t len) {
    if (!s_ws_connected || !s_ws_pcb) return false;
    ...
    if (err != ERR_OK) { printf("[ws] tcp_write header err=%d\n", err); return false; }

    err = tcp_write(s_ws_pcb, text, (u16_t)len, TCP_WRITE_FLAG_COPY);
    if (err != ERR_OK) { printf("[ws] tcp_write payload err=%d\n", err); return false; }
